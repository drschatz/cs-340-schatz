...
author:
    - Luther Tychonievich
summary: |
    <div id="due">Due dates may be found on [the schedule](../schedule.html).</div>
    <script src="ics-dates.js"></script>
    <script defer>datesFor(/MP ?3\b/i).then(showDates)</script>
...

# Overview

In this MP, you will write code with less guidance than you likely had in the past.
...
PNG files, like many other file formats, are not text-based: unlike JSON, XML, or source code you cannot simply open them in a text editor to explore their contents. You will need to consult the PNG documentation to understand the meaning of the bytes in a PNG file.
...
We recommend using this resources <https://en.wikipedia.org/wiki/PNG> for this MP. We will not summarize or teach you that content directly.


<details class="aside"><summary>Background on Chunk-based Media Files</summary>
...
<details class="aside"><summary>The two most common student questions</summary>

Question
:   How do I get started?

Answer
:   Think high level what you want the code to do, then break that into more and more detailed steps. Eventually you will have small enough steps you can start turing them into C code. 
...
Answer
:   Reflect back on the high level things you want your program to do, then see if your code is doing them. You can 
also Walk through each line in the debugger, step by step. Check that the variable values after each line makes sense. See where it is in the file when it gets to the end of your code, then ask "what comes next?" and do that.
...
You will add to the following files:
...
The first three of these will be command-line applications;
...
# Create application `pngchunklist`
...
- `fread`{.man3}
- `ntohl`{.man3}
- `printf`{.man3} or an alternative display function. Remember that if you are printing a string it needs to be null-terminated; for example a 4-byte string is stored in 5 bytes, with the last one being 0.

Coding this application shouldn't require `malloc` and should `fseek` past most of the bytes in the file.

# Create application `extractuiucchunk`
...
# Create application `insertuiucchunk`

Makes a copy of a PNG file with a new `uiuc` chunk.
...
Put a secret payload inside a PNG of your choice and share it [on the CampusWire post "Post Your MP3 Images Here!"](https://campuswire.com/c/G3A6A853A/feed/74) and [its accompanying Drive folder](https://drive.google.com/drive/folders/1Ntiagu9G8nMROaT9RlwALlCAXMn2Kzl3?q=sharedwith:public%20parent:1Ntiagu9G8nMROaT9RlwALlCAXMn2Kzl3).
This is an opportunity to share a bit about yourself and your interests;
please keep it (both the image and its hidden contents) inoffensive.

# Submission and Grading

Submit on [the submission site](../secure/upload.php).
Submit `pngchunklist.c`, `extractuiucchunk.c`, and `insertuiucchunk.c`; if you changed them, also submit `pnglib.c` and `pnglib.h`. Do not change or submit the Makefile, tests, or example images.

## Grading

Automated tests are provided in the `make test`{.sh} target.
These use specific images in the provided `img/` directory to run.
On the testing server, a different set of images will be used; that should have no impact on your grading if you've parsed the PNG files, but will reduce your score if you've [hard coded](https://en.wikipedia.org/wiki/Hard_coding) solutions to specific files or test cases.
...
- 25% extractuiucchunk
- 30% insertuiucchunk
- 10% insertuiucchunk and extractuiucchunk working together

Valgrind will also be run using

```sh
valgrind --leak-check=full --show-leak-kinds=all ./yourprogram whatever other arguments you want
```

for each of the three programs; if it gives errors other points will be reduced by 10–25%, depending on how many programs have memory errors.

<details class="aside"><summary>Debugging failed tests</summary>
