...
MP 3 - The goal of MP 3 as a whole (parts 1 and 2) is to give you practice and intuition around manipulating data on a bit level. Each part illustrates a different example.

For this part of MP 3 you will be speeding up your code from MP 1.2 using bit sets! Running MP 1.2 on my computer takes around 6 seconds to complete. The part that takes so long to run is the three nested loops towards the bottom which tries to find three 5-letter words with no overlapping letters. The big O of this part of the code is O(x^3) where x is the size of dictionary of 5-letter words. Note that the inner most three loops do not contribute meaningfully to the big O as they only loop through 5 letters. 

For this MP part you will convert the algorithm to using bit sets instead of the current approach to speed up the code without needing to change the big O! My adjusted code runs in about 2 seconds instead of 6 (what a speed up!).  
...
# Learning Goals
1. Gain an appreciation for how knowledge about how a computer works can speed up code. 
2. Be able to create and work with a bit set representation of data. 

# Background
A computer is built with logic gates which compose higher level simple operations like addition, bit shifting, and bit logic. These are often put together into more complex operations which a high level programming language can use. When you compile a C program for example, it turns the complex operations back into a series of simple operations that achieves the same logic. The more simple operations a program needs to execute its desired behavior the longer it takes to complete. This is because much of the code we write needs to be run sequentially and each operation needs time to run because of the constraints of a physical world. 
...
# Task 2 - Refactor the code to use bit sets
...
Use these two notes and what we have learned about bit sets to refactor the code to improve the speed without changing the big O. Note that the biggest speed increase will come from refactoring the logic in the triple nested loop. 

# Logistics 

## Starter Code

[`mp3.2.zip`](../mp3.2.zip) contains the same starter files as MP 1.2. Unzip it on your computer into the `cs340` directory you created during the environment setup. Then, replace the mystery.c with the contents of your completed MP 1.2 mystery.c file. Additionally, you may want to copy over any .vscode files you changed to help with debugging. 

## Running the code 
Just like MP 1.2 we aren't including a makefile or other starter files. You may use AI/Search Engines for help in how to compile, run, and start the debugger in VS Code. Feel free to look at other released MPs, to make your own Makefile, etc...
...
Within a week after the deadline I will grade your work turned in on Prairie Learn. I will grade it based on the following,
1. It produces the correct results
2. It runs quicker than my working solution of MP 1.2
3. It uses bit sets but still has the big O of O(x^3)
...
1. Do not feed AI/Search Engines any of the code in `mystery.c`. For example, do not look up: "How is this coding using sets? **paste in code" or "How do I speed up code with using a bit set?". Instead, come to office hour or post on campuswire.
