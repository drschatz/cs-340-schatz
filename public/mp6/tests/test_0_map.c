/*
 * The tests in this file are passed by the starter code;
 * as you edit and expand on that code, make sure they still pass.
 */
...
int main(int argc, char *argv[]) {
  ...
  of+=1; score += check(0xa, m_setdefault(&map, "a", 1234), "a present");
  of+=1; score += check(0xb, m_setdefault(&map, "b", 1234), "b present");
  of+=1; score += check(0xc, m_setdefault(&map, "c", 1234), "c present");
  of+=1; score += check(0xd, m_setdefault(&map, "d", 1234), "d present");
  of+=1; score += check(0xe, m_setdefault(&map, "e", 1234), "e present");
  of+=1; score += check(0xf, m_setdefault(&map, "f", 1234), "f present");
  of+=1; score += check(0xa, m_setdefault(&map, "a", 1234), "a present");
  of+=1; score += check(225, m_setdefault(&map, "g", 225), "g absent");
  m_assign(&map, "e", 0xeeee);
  of+=1; score += check(0xeeee, m_setdefault(&map, "e", 1234), "e present and changed");
