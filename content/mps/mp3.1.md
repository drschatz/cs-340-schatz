...
MP3 - The goal of MP3 as a whole (parts 1 and 2) is to give you practice and intuition around manipulating data on a bit level. Each part uses a different example.
...
- UTF-8 can encode the full Unicode range (over a million code points) rather than just the 128 code points that ASCII can encode. This now includes Greek, Arabic, emojis, mathematical symbols, and more!
...
# Task 2 - Encode
`int encodeCharacter(char **utf8, size_t *space, int codepoint)` writes the code point encoded in UTF-8 into a string pointed to by `utf8`. It then moves the string pointer past the written character and updates the remaining space. Repeated calls will fill an entire string with UTF-8 characters. More details can be found in the starter code header file.
...
You will edit `utf8lib.c` (and only that file).
...
The test suite runs tests worth 69 points. Valgrind will also be run and worth 10 points on the autograder.
...
If you submit within 24 hours after the deadline you will receive only up to 90% credit for the MP.
